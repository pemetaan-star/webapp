...
export default function Home() {
  ...
  const canSupervise = isReviewer || roleKey.includes("supervisor");
  const roleTitle = isEnumerator ? "Dashboard Enumerator" : isReviewer ? "Dashboard Koordinator & Data Analis" : "Dashboard Pendataan Hotspot";
...
function SupervisorForm({ open, user, profile, rows, onOpen, onClose, onSaved }: { open: boolean; user: User; profile: UserProfile | null; rows: Hotspot[]; onOpen: () => void; onClose: () => void; onSaved: () => void }) {
  ...
  if (!open) return <button className="supervision-launch button button-ghost" onClick={onOpen}>☑ <span>Form Supervisi</span></button>;
